...
def get_test_set(experiment_config, tokenizer):
    max_length = experiment_config['dataset']['max_length']
    df = pd.read_csv(os.path.join(experiment_config['input'], "sample_submission.csv"))
    df_ids = df["id"].unique()
    collate = Collate(tokenizer=tokenizer)

    test_samples = prepare_test_data(df, tokenizer, experiment_config['input'])

    test_dataset = FeedbackDataset(test_samples, max_length, tokenizer)
    # test_dataset = FeedbackDatasetV2(test_samples, max_length, tokenizer)
